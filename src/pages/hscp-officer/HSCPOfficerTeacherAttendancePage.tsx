...
// ─── Status styles for student view ─────────────────────
...
export default function HSCPOfficerTeacherAttendancePage() {
  ...
  useEffect(() => {
    ...
    navigate(`/hscp-officer/teacher-attendance?${params.toString()}`, { replace: true })
  ...
  // ─── Fetch student data for all grades ─────────────────
  ...
  // ─── Fetch teacher attendance when teachers loaded or date changes ─
  useEffect(() => {
  ...
  // ─── Statistics for a set of teachers ──────────────────
  ...
  // ─── Statistics for students in a section ──────────────
  ...
  const handleDownloadCSV = () => {
        ...
        gd.sections.forEach((sec) => {
          ...
          gd.students.forEach((student) => {
            ...
            csvRows.push({
              ...
              'ID/Email': student.student_identifier ?? '',
              'Attendance Date': selectedDate,
    ...
    link.href = url
    const gradeLabel = isAllGrades ? 'all-grades' : selectedGrade
    const viewLabel = viewMode === 'all' ? 'all' : viewMode
    link.setAttribute('download', `hscp-attendance-${viewLabel}-${gradeLabel}-${selectedDate}.csv`)
  ...
  // ─── Student list renderer (view-only) ─────────────────
  ...
  const renderStudentSection = (grade: string) => {
    ...
    if (!gradeData || gradeData.sections.length === 0) {
      return (
        <p className="text-muted-foreground text-center py-4 text-sm">
          No student sections found for {grade}.
        </p>
      )
    }
    ...
    const tabStats = getStudentStats(gradeData.students, secAtt)
    ...
    return (
      <div className="space-y-4">
        ...
        {/* Student list */}
        {renderStudentList(gradeData.students, secAtt)}
      </div>
    )
  }
          ...
          <div className="space-y-6">
            ...
            {showStudents && (
              <div className="space-y-4">
                ...
                {/* Embedded student card with section tabs */}
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-base text-muted-foreground mt-1">
          Record teacher attendance and view student attendance for HSCP grades.
        </p>
      </div>
      ...
      {/* Dropdowns Row: HSCP Grade + View Mode */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {/* HSCP Grade Dropdown Card */}
      ...
      {/* Pick a Date Card with Save & Download buttons */}
      <Card className="border border-[#e5e7eb] shadow-[0_2px_8px_rgba(0,0,0,0.08)] transition-all duration-[400ms] ease-[cubic-bezier(0.4,0,0.2,1)] hover:translate-y-[-4px] hover:shadow-[0_12px_24px_rgba(0,0,0,0.10)] hover:border-[#6366f1]">
        <CardHeader className="px-4 pt-3 pb-1">
      ...
      {isAllGrades && (
        <>
          {availableGrades.length === 0 ? (
            <Card>
              <CardContent className="pt-6">
                <p className="text-muted-foreground text-center">No HSCP grades found.</p>
              </CardContent>
            </Card>
          ) : (
            <div className="space-y-6">
